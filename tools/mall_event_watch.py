...
KIDS = [
    ...
    # ★**対象年齢の表記**を足した【2026-09-17・JR博多シティのこどもCITYで発覚】
    #   「ハロウィンネイルアート」は詳細ページに **対象: 未就学児〜小学生とその保護者**
    #   と明記されているのに kids_score 3 だった。「未就学児」「小学生」が語リストに無かった。
    #   対象年齢の明記は「題材が子供」ではなく「**その催しが子供向けに用意されている**」証拠
    #   なので、判定軸(家族で楽しめる仕掛けがあるか)に沿う。
    #   ⚠**語だけで拾うと誤検知する**(248件で実測して2件出た):
    #     ・「にがおえ展」… 「保育園の**園児さん**が描いてくれました」= 園児は**作者**。
    #       子供向けに用意された催しではない(ユーザーの「子供と魔法の展示」の判断と同じ)
    #     ・「福岡検定合格者限定ツアー」… 説明文の「**小学生向け検定**の新設」に当たっていた
    #   → **対象欄の文脈を要求する**。「対象〜小学生」か「小学生から参加/以下/半額」等。
    #     これで上の2件は落ち、「小学生は半額」(チン!するレストラン)や
    #     「小学生から参加OK」(謎解き)は拾える
    (6, r'(?:対象|参加者|募集)[^。]{0,24}(?:未就学児|園児|小学生|幼児)|'
        r'(?:未就学児|園児|小学生|幼児)[^。]{0,8}'
        r'(?:から参加|まで参加|以下|未満|以上|対象|無料|半額)'),
    ...
    # ★**子供向け施設の固有名**も足した【2026-09-17・JR博多シティのこどもCITYで発覚】
    #   「テレサの提灯が登場」(屋上つばめの杜ひろば・10/1〜10/31 17:00〜22:00・
    #   対象は来場者全員)は子連れ語も体験語も無く **score 0** で、
    #   `score > 0` のふるいに掛かって**一覧から丸ごと消えていた**。
    #   対象欄が「来場者全員」なので子連れ判定(6点)には届かせない。ただし
    #   **子供向け施設で開催されている**という事実は「こども館|児童館」と同種の証拠なので、
    #   ここで4点入れて「すべて」の枠には残す。展示系(天空の昆虫展)も同じ理由で残る
