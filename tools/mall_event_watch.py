...
KIDS = [
    ...
    (9, r'ワークショップ|体験|参加型|工作|つくろう|つくり|作ろう|手づくり|手作り|'
        ...
        # ★**遊具の固有名と「あそび+場所」**を足した【2026-09-16ユーザーのポスター提供で発覚】
        #   イオンモール福津の「**あそびぱ～く**」(ボールプール/ジャングルジム/おままごと・
        #   小学生以下300円)と「**パウ・パトロール フレンドパーク**」が、
        #   どちらも子連れイベントの本命なのに **kids_score 0** だった。
        #   原因は ①「遊べる/あそべる」しか無く**「あそび」に当たらない**
        #          ②遊具の固有名(ボールプール等)が語リストに無い
        #          ③「フレンドパーク」が体験語として扱われていない
        #   タイトルだけで判定するので(モールの lead は空)、**遊具の名前が入れば体験**とみなす
        r'ボールプール|ジャングルジム|トランポリン|ままごと|エア遊具|大型遊具|'
        r'あそび[ぱばパバ]|(?:フレンド|キッズ|プレイ|あそび)[\s・]{0,2}パーク|'
        # ★**フォトスポットは子連れ向けの決定要因**【2026-09-17ユーザー確定】
        #   「大きなデデデ大王の写真スポットがあるようです。こういったフォトスポットが
        #     あると子供と大人がどちらも楽しめる要素として取り入れられる。
        #     **家族で撮ることが想定されている**からです。ただの物販とは異なる点ですので、
        #     これは子供向けと判断します」
        #   → 「その場で家族が何かをする」ものなので体験系と同じ重みにする
        r'フォトスポット|写真スポット|撮影スポット|記念撮影|フォトブース|撮影コーナー|'
        r'等身大|フォトセッション|一緒に写真|'
        # ⚠**キャラクターの撮影会・グリーティングも同じ**(2026-09-17)。
        #   「ドラえもんがやってくる 撮影会」が5点で「すべて」に落ちていた。
        #   フォトスポットと同じ「**家族で撮ることが想定されている**」催しなので9点にする
        r'撮影会|グリーティング|ふれあい撮影'),
    ...
    # ⚠「遊べる/あそべる」だけでは**「あそびぱ～く」「あそび場」に当たらない**(2026-09-16)。
    #   活用形ではなく**語幹**で取る
    (5, r'遊べる|遊ぶ|遊び|あそべる|あそぼう|あそび|アソビ|さわれる|触れ|自由に'),
    (4, r'美術館|博物館|科学館|水族館|動物園|植物園|天文台|プラネタリウム|こども館|児童館'),
    # ★**天体・自然観察の催し**を足した【2026-09-16のイベント一覧の検品で発覚】
    #   青少年科学館の「市民天体観望会(月のクレーターと夏の星座)」が **kids_score 0** だった。
    #   施設名(科学館)はタイトルに出ず、語リストに「観望会」も「星座」も無かったのが原因。
    #   夜の観望会・自然観察会は子連れの定番なので体験系と同じ重みで見る
    (5, r'観望会|天体|星空|星座|望遠鏡|観察会|自然観察|いきもの観察|生きもの観察'),
    # ★**子どもへの呼びかけ文体**。「〜しよう」で終わる催しは参加型であることのサイン
    (3, r'たのしもう|楽しもう|やってみよう|みつけよう|見つけよう|さがそう|探そう|'
        r'つくってみよう|さわってみよう|のぞいてみよう'),
    # ⚠IPの語リストは**実際に館で開催されたものを見たら足す**。2026-09-16に
    #   「パウ・パトロール」(ニコロデオン。未就学児に強い)が入っておらず0点だった
    (5, r'ふれあい|握手|ショー|ステージ|ヒーロー|戦隊|仮面ライダー|プリキュア|'
        r'ポケモン|ちいかわ|サンリオ|アンパンマン|しまじろう|ドゲンジャーズ|ドラえもん|'
        r'パウ.?パトロール|パウパト|シナモロール|マイメロ|きかんしゃトーマス|'
        # ⚠任天堂系が抜けていた(2026-09-17)。「星のカービィ デデデ★デパート」が0点だった。
        #   ⚠ただし**IPだけでは子連れ向けにしない**(ユーザー基準)。
        #     「基本的には物販なのですべてで良い」= 5点では閾値6に届かない、が正しい挙動
        r'カービィ|デデデ|ワドルディ|マリオ|ゼルダ|ピクミン|スプラトゥーン|どうぶつの森|'
        r'トミカ|プラレール|シルバニア|リカちゃん|ミニオン|妖怪ウォッチ|ムーミン|'
        # シャボン玉のショー(イオンモール福津「サンセットバブル／ナイトバブル」)。
        # タイトルに「シャボン玉」が出ず「バブル」だけなので両方取る
        r'シャボン玉|バブル'),
    ...
    # ⚠**花火は「花火大会」という語が無いと0点だった**【2026-09-17に実測】
    #   県公式のタイトルは「行橋夏まつり こすもっぺ」「ゆいきらら とうろう祭」で、
    #   lead に「**2,500発の打ち上げ花火**で盛り上がる！」「**打ち上げ花火**も初開催」と
    #   書いてあるのに `花火大会` に当たらず ks=4 のままだった(一覧の下位に沈んでいた)。
    #   → **打ち上げ花火**と**打ち上げ数(◯◯発)**を足す。発数は大型花火の明確な指標
    (12, r'大祭|例大祭|三大祭|放生会|山笠|どんたく|くんち|おくんち|花火大会|'
         r'打ち上げ花火|打上花火|奉納花火|花火の祭典|[0-9０-９][0-9０-９,，]{2,}\s*発|'
         # 市を代表する規模の祭り。放生会と同じ扱い(固有名で足すしかないもの)
         r'百万夏まつり|水の祭典|わっしょい|'
         r'大祭典|大花火|灯明|燈籠|神幸|祇園'),
    # ★**回数は規模の代理指標**。第20回以上まで続いている行事は地域の定番
    #   (「第55回 水の祭典久留米まつり」「第102回 飯塚納涼花火大会」「第27回 ISLA DE SALSA」)
    (6, r'第[2-9][0-9]回|第[1-9][0-9][0-9]回'),
    (5, r'フェスタ|大会$|選手権'),
...
# ★大人向け(飲酒が主題)の語。kids_score で減点に使う【2026-09-16】
NOT_KIDS = re.compile(r'ビアガーデン|ビアフェス|ビール|地酒|日本酒|焼酎|ワイン|'
                      r'酒フェス|バル街|飲み放題|居酒屋|ナイトバー|立ち飲み|'
                      # ⚠裸の「バー」は**バーベキュー/ハンバーガー**に当たるので入れない
                      r'大人限定|20歳以上|お酒')

# ★**「題材が子供」だけでは子連れ向けにしない**【2026-09-17ユーザー確定】
#   「子供と魔法の展示については**題材が子供**ということで子供向けに入っているかと思います。
#     内容としては古典で**子供を対象とした施設が用意されているわけではない開催場所**です。
#     ということはすべて、のジャンルに入るものかと思います」
#   → 個展・展覧会のような**見るだけ**のものは、体験・フォトスポット・遊具が何も無ければ
#     子連れ語(こども/キッズ/親子)の加点を**打ち消す**。
#   ⚠「もっと！こどもの**視展**」(体験12種)や「魔法の美術館」(見て触れて遊ぶ)のように
#     体験が伴うものは打ち消さない。だから **`展` 全般ではなく「個展・展覧会」等に限定**する
#     (「◯◯展」を全部入れると、こどもの視展やジブリ展まで落ちてしまう)
VIEW_ONLY = re.compile(r'個展|展覧会|絵画展|写真展|作品展|回顧展|書展|原画展|'
                       r'油彩|日本画|洋画|陶芸展|工芸展|彫刻展')
...
def kids_score(title):
    ...
    t = re.sub(r'[\s　]+', '', title)
    s = sum(w for w, p in KIDS if re.search(p, t))
    # ★**飲酒が主題の催しは子連れ判定から外す**【2026-09-16のイベント一覧の検品で発覚】
    #   「皿倉山展望台レストランTEN-KYU 星空ビアガーデン」が **星空(+5)** で
    #   子連れ向けに紛れ込んだ。ただし「親子ビール工場見学」のように
    #   **明示的な子連れ語(KIDS[0])がある場合は減点しない**(そちらの語のほうが強い証拠)
    if s and not re.search(KIDS[0][1], t) and NOT_KIDS.search(t):
        s = 0
    # ★**「題材が子供」だけの見るだけの展示は子連れ向けにしない**【2026-09-17ユーザー確定】
    #   杉本さなえ個展「こどもとまほう Child and Magic」が「こども」(+6)+「展」(+2)=8点で
    #   子連れ向けに入っていた。ユーザーの判断は
    #   「題材が子供ということで子供向けに入っているが、内容は古典で
    #     **子供を対象とした施設が用意されているわけではない**開催場所。すべてのジャンル」
    #   → 個展・展覧会で、**体験系(KIDS[1]=ワークショップ/フォトスポット/遊具など)が
    #     何も無い**なら、子連れ語と「展」の加点を打ち消す
    if VIEW_ONLY.search(t) and not re.search(KIDS[1][1], t):
        s -= 8                      # 子連れ語(6) + 展(2) を相殺する
    return max(0, s)
